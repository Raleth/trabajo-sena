    ...
    })
    {
        CLAY(CLAY_ID("sidebar_title"), {
            ...
            CLAY_TEXT(Clay_MakeString(get_text("TITULO_VENTANA")), CLAY_TEXT_CONFIG({
                .fontId = fontId, .fontSize = 24, .textColor = t->texto
            }));
        }
        ...
        CLAY(CLAY_ID("amount_label"), {
            ...
            CLAY_TEXT(Clay_MakeString(get_text("CANTIDAD")), CLAY_TEXT_CONFIG({
                .fontId = fontId, .fontSize = 16, .textColor = t->texto
            }));
        }

        CLAY(ids->amountInput, {
            .backgroundColor = uiState.amountInputActive ?
                (Clay_Color){75, 75, 85, 255} : t->input,
            .cornerRadius = 4,
            .layout = {
            ...
            CLAY_TEXT(Clay_MakeString(amountDisplayText), CLAY_TEXT_CONFIG({
                .fontId = fontId, .fontSize = 18, .textColor = t->texto
            }));
        }
        ...
        CLAY(ids->btnIncome, {
            .backgroundColor = Clay_Hovered() ? t->botonPrimaryHover : t->botonPrimary,
            .cornerRadius = 6,
            .layout = {
                .sizing = {.width = CLAY_SIZING_GROW(), .height = CLAY_SIZING_FIXED(40)},
                .padding = {10, 10, 10, 10},
            ...
            CLAY_TEXT(Clay_MakeString(get_text("AÑADIR_INGRESO")), CLAY_TEXT_CONFIG({
                .fontId = 0, .fontSize = 18, .textColor = {255, 255, 255, 255}
            }));
        }

        CLAY(ids->btnExpense, {
            .backgroundColor = Clay_Hovered() ? t->botonDangerHover : t->botonDanger,
            .cornerRadius = 6,
            .layout = {
                .sizing = {.width = CLAY_SIZING_GROW(), .height = CLAY_SIZING_FIXED(40)},
                .padding = {10, 10, 10, 10},
    ...
    })
    {
        ...
        })
        {
            CLAY_TEXT(Clay_MakeString(get_text("TOTAL_INGRESOS")), CLAY_TEXT_CONFIG({
                .fontId = fontId, .fontSize = 16, .textColor = {220, 255, 220, 255}
            }));
            ...
            CLAY_TEXT(Clay_MakeString(uiState.incomeText), CLAY_TEXT_CONFIG({
                .fontId = fontId, .fontSize = 32, .textColor = {255, 255, 255, 255}
            }));
        }
        ...
        })
        {
            CLAY_TEXT(Clay_MakeString(get_text("TOTAL_GASTOS")), CLAY_TEXT_CONFIG({
                .fontId = fontId, .fontSize = 16, .textColor = {255, 220, 220, 255}
            }));
        ...
        })
        {
            CLAY_TEXT(Clay_MakeString(get_text("BALANCE_NETO")), CLAY_TEXT_CONFIG({
                .fontId = fontId, .fontSize = 16, .textColor = {220, 220, 255, 255}
            }));
...
static void RenderTransactions(int fontId, const UITheme *t) {
    CLAY(CLAY_ID("transactions_title"), {
        ...
        CLAY_TEXT(Clay_MakeString(titleText), CLAY_TEXT_CONFIG({
            .fontId = fontId, .fontSize = 22, .textColor = t->texto
        }));
    }
        ...
        if (budget.count == 0) {
            ...
            for (int i = budget.count - 1; i >= 0 && i >= budget.count - 20; i--) {
                ...
                CLAY(Clay__HashNumber(itemHashId, 0), {
                    .backgroundColor = tnx->type == TRANSACTION_INCOME ?
                ...
                })
                {
                    CLAY(Clay__HashNumber(itemHashId + 1000, 0), {
                        ...
                        CLAY_TEXT(Clay_MakeString(tnx->description), CLAY_TEXT_CONFIG({
                            .fontId = fontId, .fontSize = 16, .textColor = t->texto
                        }));

                        CLAY_TEXT(Clay_MakeString(tnx->date), CLAY_TEXT_CONFIG({
                            .fontId = fontId, .fontSize = 12,
...
static void RenderWorkspace(const UITheme *t, int fontId) {
        ...
        if (uiState.statusMessageTime > 0) {
            CLAY(CLAY_ID("status_message"), {
                ...
                CLAY_TEXT(Clay_MakeString(uiState.statusMessage), CLAY_TEXT_CONFIG({
                    .fontId = fontId, .fontSize = 16, .textColor = {255, 255, 255, 255}
                }));
            }
        }
